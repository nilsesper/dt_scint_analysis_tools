...
def _fit_gaussian_window(
    ...
    for attempt in range(max_attempts):
        fit_mask = (bins_nobg >= mu_est - win * sigma_est) & (bins_nobg <= mu_est + win * sigma_est)
        ...
        try:
            a2_est, a1_est, a0_est = np.polyfit(edge_x, edge_y, 2)
        except np.linalg.LinAlgError:
            a2_est, a1_est, a0_est = 0.0, 0.0, np.median(fit_hist)

        p0 = (A_est, mu_est, sigma_est, a2_est, a1_est, a0_est)
        lower = [0, fit_bins.min(), 1e-6, -np.inf, -np.inf, -np.inf]
        upper = [np.inf, fit_bins.max(), (fit_bins.max() - fit_bins.min()), np.inf, np.inf, np.inf]

        try:
            popt, pcov = curve_fit(
                gauss_plus_quad, fit_bins, fit_hist,
            ...
            fit_vals = gauss_plus_quad(fit_bins, *popt)
            ...
            return popt, pcov, fit_bins, fit_hist, err_fit_hist, gauss_plus_quad
...
def fit_secondary_peak(
    ...
    if search_min is not None:
        # explicit region given -- behave as a single-candidate fit
        mu_est, sigma_est, A_est = find_secondary_peak(
            bins_nobg, hist_nobg,
            min_x=search_min, max_x=search_max if search_max is not None else bins_nobg.max(),
...
def main():
    ...
    dataset_name_list = ["cosmic_82-18_3600-1800-1200_test1_th20", "cosmic_82-18_3600-1800-1200_test2_th20",
    ...
    fit_err = err_gauss_plus_quad(fit_bins, *popt, *perr)
    ...
    #when looping over datasets, stop indentation here
    vds = []
    errs_vd = []
    data_sets = []
    for dataset in results_dict:
        vd = results_dict[dataset][fit_params]["mu"]
        err_vd = results_dict[dataset][perr]["mu"]
        vds.append(vd)
        errs_vd.append(err_vd)
