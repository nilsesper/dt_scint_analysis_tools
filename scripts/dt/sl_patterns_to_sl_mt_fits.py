...
def main():
    ...
    if do_multiprocessing: # with multiprocessing
        sl_fits = process_utils.multiprocess_data(n_processes=n_processes, n_batches=n_batches_sl_fitting, function=dt_utils.fit_sl_patterns_meantimer, data=sl_patterns, data_key="patterns", kwargs={}, mute=True)
    else: # without multiprocessing
        sl_fits = dt_utils.fit_sl_patterns_meantimer(patterns=sl_patterns, verbose=verbose)
