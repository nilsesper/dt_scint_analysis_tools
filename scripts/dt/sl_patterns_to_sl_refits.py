...
def main():
    ...
    if do_multiprocessing: # with multiprocessing
        sl_fits = process_utils.multiprocess_data(n_processes=n_processes, n_batches=n_batches_sl_fitting, function=dt_utils.fit_sl_patterns, data=sl_patterns, data_key="patterns", kwargs={"fit_vd": fit_vd, "suffix": ""}, mute=True)
    else: # without multiprocessing
        sl_fits = dt_utils.fit_sl_patterns(patterns=sl_patterns, verbose=verbose, fit_vd=fit_vd, suffix="")
        # cut data to restrict to chi2/ndf < 10 and |alpha| < 10 deg
        max_alpha = np.deg2rad(10)
        max_chi2 = 10
        sl_cut_fits = data_utils.cutdata(data=sl_fits, conditions=[("impossible","==",0), ("chi2/ndf","<",max_chi2), ("alpha","in", (-np.deg2rad(max_alpha), np.deg2rad(max_alpha)))], silent=True)  
        sl_refits = dt_utils.fit_sl_patterns(patterns=sl_cut_fits, silent=True, verbose=verbose, fit_vd=True, suffix="refit")
    ...
    sl_patterns_refit = data_utils.sort_by_key(data=sl_refits, sort_key="t0")
    ...
    print(f"###### Storing SL-level refits to file \"{sl_fits_file + '_refits'}\"...")
    data_utils.store_pickle(data=sl_refits, file=sl_fits_file + "_refits")
