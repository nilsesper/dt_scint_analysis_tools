...
def main():
    ...
    for i in range(n_muons):
        cosmic_muons.append( muon_utils.generate_cosmic_muon(xrange=[-100, 2300], yrange=[-100, 2600], z0=100, ts=t_step*(i+1)) )
    ...
    # plot muon track
    for i in range(n_muons):
        ax = geoplot_utils.muon_ax(ax=ax, orient=orient, muon=cosmic_muons[i], color="tab:green")
    # axis limits
    ax.margins(x=0.05, y=0.05)
    # text labels
    axbox = ax.get_position()
