...
def cell_pat(orient, sl, ly, wi, *, wire=False, cell_data=None, transparent=False): # sliced cell data for this cell {color, text}
    ...
    if not transparent:
        edgecolor = params._color_info["cell"]["edge"]
        facecolor = cell_data["color"] #params._color_info["cell"][None]
        zorder = 1
        if cell_data["color"] != params._color_info["cell"][None]:
            zorder = 2
        facecolor_side = params._color_info["cell"]["side_view"]
        facecolor_wire_side = params._color_info["cell"]["wire"]
        facecolor_wire = params._color_info["cell"]["wire"]
    else:
        edgecolor = cell_data["color"]
        zorder = 1
        if cell_data["color"] != params._color_info["cell"][None]:
            zorder = 2
        facecolor = "none"
        facecolor_side = "none"
        facecolor_wire_side = "none"
        facecolor_wire = "white"
    ## if the orientation matches the wire direction, draw all cells
    if orient == params._dt_chamber["sls"][sl]["orient"]:
        patches.append( pat.Rectangle((derived_params._dt_cell_coordinates[sl][ly][wi][x_axis][0], derived_params._dt_cell_coordinates[sl][ly][wi][y_axis][0]), width=(derived_params._dt_cell_coordinates[sl][ly][wi][x_axis][1]-derived_params._dt_cell_coordinates[sl][ly][wi][x_axis][0]), height=(derived_params._dt_cell_coordinates[sl][ly][wi][y_axis][1]-derived_params._dt_cell_coordinates[sl][ly][wi][y_axis][0]), edgecolor=edgecolor, facecolor=facecolor, zorder=zorder) )
        if wire:
            ...
            patches.append( pat.Circle((wire_x, wire_y), radius=params._dt_chamber["sls"][sl]["wi_radius"], edgecolor=None, facecolor=facecolor_wire ) )
    ...
    # always display the cells with None value, since behind the drawn wi=0 are many others and it is simpler to not draw any data
        ...
        # cell color
        patches.append( pat.Rectangle((derived_params._dt_cell_coordinates[sl][ly][wi][x_axis][0], derived_params._dt_cell_coordinates[sl][ly][wi][y_axis][0]), width=(derived_params._dt_cell_coordinates[sl][ly][wi][x_axis][1]-derived_params._dt_cell_coordinates[sl][ly][wi][x_axis][0]), height=(derived_params._dt_cell_coordinates[sl][ly][wi][y_axis][1]-derived_params._dt_cell_coordinates[sl][ly][wi][y_axis][0]), edgecolor=edgecolor, facecolor=facecolor_side) )
        # wire (line)
        if wire:
            patches.append( pat.Polygon([(derived_params._dt_cell_coordinates[sl][ly][wi][x_axis][0], derived_params._dt_cell_coordinates[sl][ly][wi][3+y_axis]), (derived_params._dt_cell_coordinates[sl][ly][wi][x_axis][1], derived_params._dt_cell_coordinates[sl][ly][wi][3+y_axis])], linewidth=params._dt_chamber["sls"][sl]["wi_linewidth"], edgecolor=facecolor_wire_side, facecolor=None, closed=False, visible=True) ) # params._color_info["cell"]["wire"]
    return patches # return list of mpl patches

# draw one layer as list of patches
def layer_pat(orient, sl, ly, *, wire=False, cell_data=None, transparent=False): # sliced cell data for this layer {wi: {color, text}}
    x_axis, y_axis = params._orientation[orient][0], params._orientation[orient][1] # chamber axis projection (x,y,z) to 2D plot axis (x,y)
    patches = []
    # cells
    for wi in range(params._dt_chamber["sls"][sl]["lys"][ly]["min_wi"], params._dt_chamber["sls"][sl]["lys"][ly]["max_wi"]+1):
        patches.extend( cell_pat(orient=orient, sl=sl, ly=ly, wi=wi, wire=wire, cell_data=cell_data[wi], transparent=transparent) )
    return patches # return list of mpl patches

# draw one superlayer as list of patches
def superlayer_pat(orient, sl, *, wire=False, cell_data=None, transparent=False): # sliced cell data for this superlayer {ly: {wi: {color, text}}}
    x_axis, y_axis = params._orientation[orient][0], params._orientation[orient][1] # chamber axis projection (x,y,z) to 2D plot axis (x,y)
    patches = []
    # sl casing
    if not transparent:
        edgecolor=params._color_info["sl"]["edge"]
        facecolor=params._color_info["sl"]["fill"]
    else:
        edgecolor="white"
        facecolor="none"
    patches.append( pat.Rectangle((params._dt_chamber["pos"][x_axis]+params._dt_chamber["sls"][sl]["pos"][x_axis], params._dt_chamber["pos"][y_axis]+params._dt_chamber["sls"][sl]["pos"][y_axis]), width=params._dt_chamber["sls"][sl]["size"][x_axis], height=params._dt_chamber["sls"][sl]["size"][y_axis], facecolor=facecolor, edgecolor=edgecolor) )
    # layers
...
def chamber_pat(orient, *, wire=False, cell_data=None, transparent=False):  # sliced cell data for this chamber {sl: {ly: {wi: {color, text}}}}
    x_axis, y_axis = params._orientation[orient][0], params._orientation[orient][1] # chamber axis projection (x,y,z) to 2D plot axis (x,y)
    patches = []
    ## chamber casing
    ...
    for sl in range(1,params._dt_chamber["n_sl"]+1):
        patches.extend( superlayer_pat(orient=orient, sl=sl, wire=wire, cell_data=cell_data[sl], transparent=transparent) )
    return patches # return list of mpl patches
...
def chamber_ax(ax, orient, cell_data, *, wire=False, transparent=False):
    patches = chamber_pat(orient=orient, wire=wire, cell_data=cell_data, transparent=transparent)
...
def scintillator_strip_pat(orient, ly, st, *, cell_data=None, transparent=False): # sliced cell data for this cell {color, text}
    x_axis, y_axis = params._orientation[orient][0], params._orientation[orient][1] # chamber axis projection (x,y,z) to 2D plot axis (x,y)
    patches = []
    # emtpy cell struct if none is given
    if cell_data == None: cell_data = {"color": params._color_info["cell"][None], "text": ""}
    ...
    if orient == params._scintillator["lys"][ly]["orient"]:
        ...
        patches.append( pat.Rectangle((derived_params._scintillator_strip_coordinates[ly][st][x_axis][0], derived_params._scintillator_strip_coordinates[ly][st][y_axis][0]), width=(derived_params._scintillator_strip_coordinates[ly][st][x_axis][1]-derived_params._scintillator_strip_coordinates[ly][st][x_axis][0]), height=(derived_params._scintillator_strip_coordinates[ly][st][y_axis][1]-derived_params._scintillator_strip_coordinates[ly][st][y_axis][0]), edgecolor=edgecolor, facecolor=facecolor, zorder=zorder) )
    ## if orientation is flipped, only outline the cells
    # in order to prevent overlaying objects in the plot only draw wi=0 for each layer from the side
    # always display the cells with None value, since behind the drawn wi=0 are many others and it is simpler to not draw any data
        ...
        patches.append( pat.Rectangle((derived_params._scintillator_strip_coordinates[ly][st][x_axis][0], derived_params._scintillator_strip_coordinates[ly][st][y_axis][0]), width=(derived_params._scintillator_strip_coordinates[ly][st][x_axis][1]-derived_params._scintillator_strip_coordinates[ly][st][x_axis][0]), height=(derived_params._scintillator_strip_coordinates[ly][st][y_axis][1]-derived_params._scintillator_strip_coordinates[ly][st][y_axis][0]), edgecolor=edgecolor, facecolor=facecolor_side, zorder=zorder) )
    return patches # return list of mpl patches

# draw one scintillator layer as list of patches
def scintillator_layer_pat(orient, ly, *, cell_data=None, transparent=False): # sliced cell data for this layer {wi: {color, text}}
    x_axis, y_axis = params._orientation[orient][0], params._orientation[orient][1] # chamber axis projection (x,y,z) to 2D plot axis (x,y)
    patches = []
    # strips
    for st in range(params._scintillator["lys"][ly]["n_sts"]):
        patches.extend( scintillator_strip_pat(orient=orient, ly=ly, st=st, cell_data=cell_data[st], transparent=transparent) )
    return patches # return list of mpl patches

# draw one scintillator as list of patches
def scintillator_pat(orient, *, cell_data=None, transparent=False):  # sliced cell data for this chamber {sl: {ly: {wi: {color, text}}}}
    x_axis, y_axis = params._orientation[orient][0], params._orientation[orient][1] # chamber axis projection (x,y,z) to 2D plot axis (x,y)
    patches = []
    if not transparent:
        edgecolor=params._color_info["sl"]["edge"]
        ...
        facecolor="none"
    # chamber casing
    patches.append( pat.Rectangle((params._scintillator["pos"][x_axis], params._scintillator["pos"][y_axis]), width=params._scintillator["size"][x_axis], height=params._scintillator["size"][y_axis], edgecolor=edgecolor, facecolor=facecolor) )
    # layers
    for ly in range(params._scintillator["n_lys"]):
        patches.extend( scintillator_layer_pat(orient=orient, ly=ly, cell_data=cell_data[ly], transparent=transparent) )
    return patches # return list of mpl patches

### draw scintillator into existing ax (subplot)
# cell_data: {ly: {st: {"color": color, "text": text}}}
def scintillator_ax(ax, orient, cell_data, transparent=False):
    patches = scintillator_pat(orient=orient, cell_data=cell_data, transparent=transparent)
    for patch in patches:
        ax.add_patch(patch)
    return ax
